...
WHERE
	--Clean Data: (PRL02 in (‘DRV’) AND (EMDCG NOT LIKE ‘V%’ AND EMDCG NOT LIKE ‘D%’)) AND PRL04 = EMTE
	...
	AND (
			(
				PRPMS.PRL02 IN('DRV') -- in SBU
				AND (PMEMPLP.EMDCG LIKE 'V%' or PMEMPLP.EMDCG LIKE 'D%') --not In Control Group
			)

			OR (
				PRPMS.PRL02 NOT IN('DRV', 'EXD') -- not in SBU
				AND (PMEMPLP.EMDCG NOT LIKE 'V%'AND PMEMPLP.EMDCG NOT LIKE 'D%') --not In Control Group
			)

			OR (
				(PRPMS.PRL02 IN ('DRV') OR (PMEMPLP.EMDCG LIKE 'V%' AND PMEMPLP.EMDCG LIKE 'D%')) --In SBU or Control Group
